function [p1,p2,d] = MatchPlot(img1, img2, feature, threshold)
  ...
  if ~strcmpi(feature,'sift')
    f1 = FeatureDetect(img1, [3,40]);
    f2 = FeatureDetect(img2, [3,40]);
    %f1(3,:) = f1(3,:) * 4;
    %f2(3,:) = f2(3,:) * 4;
    d1 = NeuralFeature(img1, f1);
    d2 = NeuralFeature(img2, f2);
  end
  [m,d] = vl_ubcmatch(d1, d2, threshold);
  p1 = f1(1:2, m(1,:))';
  p2 = f2(1:2, m(2,:))';
  % h = MatchPlotImp(img1, img2, p1, p2);
end

function h = MatchPlotImp(img1,img2,points1,points2)
%MATCH_PLOT
