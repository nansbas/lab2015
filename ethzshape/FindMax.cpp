...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (!maxv.SetOutput(0, in.dims, mxDOUBLE_CLASS)) return;
  if (!maxm.SetOutput(1, in.dims, mxLOGICAL_CLASS)) return;
...
void findMaximum()
{
  ...
  for (int j = 0; j < in.w; j++) {
    ...
    for (int i = in.h - 1; i >= 0; i--) {
      if (i % gap == gap - 1 || i == in.h - 1 || maxv(i,j) > g[i+1]) { g[i] = maxv(i,j); t[i] = maxm(i,j); }
      else if (maxv(i,j) < g[i+1]) { g[i] = g[i+1]; t[i] = t[i+1]; }
      else { g[i] = maxv(i,j); t[i] = 1; }
    }
    for (int i = 0; i < in.h; i++) {
      int a = i - neighbor, b = i + neighbor;
      if (a < 0 || (g[a] < h[b] && b < in.w)) { maxv(i,j) = h[b]; maxm(i,j) = r[b]; }
      else if (b >= in.w || (g[a] > h[b] && a >= 0)) { maxv(i,j) = g[a]; maxm(i,j) = t[a]; }
      else { maxv(i,j) = g[a]; maxm(i,j) = 1; }
    }
  ...
  for (int i = 0; i < in.h; i++) for (int j = 0; j < in.w; j++) {
    ...
    else if (maxv(i,j) == in(i,j) && maxm(i,j) == 0) maxm(i,j) = 1;
    else maxm(i,j) = 0;
  }
}
